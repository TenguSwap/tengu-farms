...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingTengu(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accTenguPerShare = accTenguPerShare.add(tenguReward.mul(1e12).div(lpSupply));
        }
    ...
    function _updatePool(uint256 _pid) internal {
        ...
        pool.accTenguPerShare = pool.accTenguPerShare.add(tenguReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(tengu)) {
                uint256 transferTax = _amount.mul(tengu.transferTaxRate()).div(10000);
    ...
    function updateEmissionRate(uint256 _tenguPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, tenguPerBlock, _tenguPerBlock);
        tenguPerBlock = _tenguPerBlock;
    }
